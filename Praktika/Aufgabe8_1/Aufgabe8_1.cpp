...
void draw_boxes(Box boxes[]) {
	...
	// Farbe: blue
	// TO DO
	for (int index = 0; index < box_max; index++) {
		gip_draw_rectangle(boxes[index].x, boxes[index].y, boxes[index].x + box_size, boxes[index].y + box_size, blue);
	}
}
// update_boxes():
...
bool update_boxes(Box boxes[]) {
	// TO DO
	for (int index = 0; index < box_max; index++) {
		boxes[index].y = boxes[index].y + boxes[index].delta_y;

		if (boxes[index].y >= gip_win_size_y) {
			return false;
		} 
	}
	return true;
...
void handle_mouse_click(Box boxes[]) {
	...
	for (int index = 0; index < box_max; index++) {
		if ((mouse_x >= boxes[index].x && mouse_x <= boxes[index].x + box_size) && (mouse_y >= boxes[index].y && mouse_y <= boxes[index].y + box_size)) {
			boxes[index].delta_y += 10;
			boxes[index].y = 0;
 		}
	}
}
...
int main() {
	...
	for (int index = 0; index < box_max; index++) {
		...
		boxes[index].x = index * (box_size + 20) + 10;
		boxes[index].y = 0;
		// Startgeschwindigkeit ist ganzzahlige Zufallszahl zwischen 10 und 30 ...
		boxes[index].delta_y = gip_random(10, 30);
	}
